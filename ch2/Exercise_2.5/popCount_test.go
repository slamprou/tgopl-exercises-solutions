...
func clearRightmostNonZeroPopCount(x uint64) int {
	count := 0
	for x > 0 {
		x = x & (x - 1)
		count++
...
func BenchmarkClearRightmostNonZeroPopCount(b *testing.B) {
	for i := 0; i < b.N; i++ {
		clearRightmostNonZeroPopCount(0x1234567890ABCDEF)
	}
}
